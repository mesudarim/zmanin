<script setup lang="ts">
import { ref } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import { useAuthStore } from '@/stores/auth'
import { useI18nStore } from '@/stores/i18n'
import AppButton from '@/components/ui/AppButton.vue'
import LanguageSwitcher from '@/components/ui/LanguageSwitcher.vue'

const auth = useAuthStore()
const i18n = useI18nStore()
const router = useRouter()
const route = useRoute()

const t = i18n.t
const email = ref('')
const linkSent = ref(false)
const loading = ref(false)
const error = ref('')

async function loginGoogle() {
  loading.value = true
  error.value = ''
  try {
    await auth.loginWithGoogle()
    redirect()
  } catch (e: unknown) {
    error.value = (e as Error).message
  } finally {
    loading.value = false
  }
}

async function sendLink() {
  if (!email.value) return
  loading.value = true
  error.value = ''
  try {
    await auth.sendEmailLink(email.value)
    linkSent.value = true
  } catch (e: unknown) {
    error.value = (e as Error).message
  } finally {
    loading.value = false
  }
}

function redirect() {
  const dest = (route.query.redirect as string) ?? (auth.isAdmin ? '/admin' : '/dashboard')
  router.push(dest)
}
</script>

<template>
  <div class="min-h-screen bg-gradient-to-br from-primary-50 via-white to-primary-100 flex items-center justify-center p-4">
    <div class="w-full max-w-sm">
      <!-- Language switcher -->
      <div class="flex justify-end mb-6">
        <LanguageSwitcher />
      </div>

      <div class="card">
        <!-- Logo -->
        <div class="text-center mb-8">
          <div class="inline-flex items-center justify-center w-14 h-14 bg-primary-600 rounded-2xl mb-4">
            <span class="text-white text-2xl font-black">Z</span>
          </div>
          <h1 class="text-2xl font-bold text-gray-900">ZmanIn</h1>
          <p class="text-sm text-gray-500 mt-1">{{ t.auth.title }}</p>
        </div>

        <div v-if="linkSent" class="text-center py-4">
          <div class="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-3">
            <svg class="w-6 h-6 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
            </svg>
          </div>
          <p class="text-sm text-gray-600">{{ t.auth.sendLinkSuccess }}</p>
        </div>

        <div v-else class="space-y-4">
          <!-- Google -->
          <AppButton variant="secondary" size="lg" class="w-full" :loading="loading" @click="loginGoogle">
            <svg class="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            {{ t.auth.signInGoogle }}
          </AppButton>

          <div class="relative flex items-center gap-3">
            <div class="flex-1 h-px bg-gray-200" />
            <span class="text-xs text-gray-400">or</span>
            <div class="flex-1 h-px bg-gray-200" />
          </div>

          <!-- Email link -->
          <div class="space-y-3">
            <div>
              <label class="text-sm font-medium text-gray-700 block mb-1">{{ t.auth.emailLabel }}</label>
              <input
                v-model="email"
                type="email"
                :placeholder="t.auth.emailPlaceholder"
                class="block w-full rounded-xl border-gray-300 shadow-sm text-sm focus:border-primary-500 focus:ring-primary-500"
                @keyup.enter="sendLink"
              />
            </div>
            <AppButton size="lg" class="w-full" :loading="loading" @click="sendLink">
              {{ t.auth.signInEmail }}
            </AppButton>
          </div>

          <p v-if="error" class="text-xs text-red-500 text-center">{{ error }}</p>

          <p class="text-xs text-gray-400 text-center">{{ t.auth.noAccount }}</p>
        </div>
      </div>
    </div>
  </div>
</template>
